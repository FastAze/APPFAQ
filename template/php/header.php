<?php
    include 'ini.php';
?>
<header>
    <h1>
        <?php
            echo $_SESSION["username"];
            echo $_SESSION["ligue"]; // il manque le SQL pour afficher la ligue et le type d'utilisateur
        ?>
    </h1>
    <nav>
        <ul>
            <li><a href="/pages/list.php">Liste des questions</a></li>
            <li><a href="/pages/add.php">Ajouter une question</a></li>
            <li><a href="/pages/editmes.php">Modifier une question</a></li>
            <li><a href="/pages/editrep.php">Modifier une réponse</a></li>
            <li><a href="/pages/delete.php">Supprimer une question</a></li>
        </ul>
    </nav>
    <button class="floating-button" onclick="window.location.href='/pages/logout.php'">Déconnexion</button>
</header>